...
extension AppDelegate: OTSessionDelegate {
    func sessionDidConnect(_ sessionDelegate: OTSession) {
        ...
        if let pubView = self.publisher.view {
            pubView.frame = CGRect(x: 0, y: 0, width: 150, height: 200)
...
extension AppDelegate: OTSubscriberDelegate {
    func subscriberDidConnect(toStream subscriberKit: OTSubscriberKit) {
        ...
        if let subView = self.subscriber?.view {
            subView.frame = CGRect(x: 0, y: 0, width: UIScreen.main.bounds.width, height: UIScreen.main.bounds.height*0.8)
...
final class OpenTokVideoContainer: UIView {
    private let subscriberContainer = UIStackView()
    private let publisherContainer = UIStackView()
    ...
    override func layoutSubviews() {
        ...
        publisherContainer.frame = CGRect(x: width*0.58, y: height*0.75, width: width, height: height)
    }
}
